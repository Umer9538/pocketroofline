...
actor LlamaContext {
    ...
    // PocketRoofline: one measured repeat of a (promptTokens, generateTokens)
    ...
    func prBenchOnce(promptTokens rawPrompt: Int, generateTokens: Int) -> (prefill: Double, decode: Double, ttftMs: Double) {
        ...
        if llama_decode(context, batch) != 0 { print("PR: decode failed (prefill)") }
        llama_synchronize(context)
        let tPpEnd = DispatchTime.now().uptimeNanoseconds

        // --- decode (single-stream generation) ---
        llama_memory_clear(llama_get_memory(context), false)
        let tTgStart = DispatchTime.now().uptimeNanoseconds
        for i in 0..<generateTokens {
            llama_batch_clear(&batch)
            llama_batch_add(&batch, 0, Int32(i), [0], true)
            if llama_decode(context, batch) != 0 { print("PR: decode failed (gen)") }
            llama_synchronize(context)
        }
        let tTgEnd = DispatchTime.now().uptimeNanoseconds
        llama_memory_clear(llama_get_memory(context), false)

        let tPpS = Double(tPpEnd - tPpStart) / 1e9
        let tTgS = Double(tTgEnd - tTgStart) / 1e9
        let prefill = Double(promptTokens) / tPpS
        let decode = Double(generateTokens) / tTgS
        let ttftMs = tPpS * 1000.0
        return (prefill, decode, ttftMs)
    }

    func prModelParams() -> Double { Double(llama_model_n_params(model)) / 1e9 }
    func prModelSizeBytes() -> Int { Int(llama_model_size(model)) }
